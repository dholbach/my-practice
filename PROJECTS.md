...
- **2026-08-19 — v0.5.0 minor release**: three features (free-form/non-session invoice items completing P-122 Phase 1, #335–#337; learned expense auto-categorization from counterparty history, #338; form draft guard against accidental navigation on long-text forms, #339), a Django 6.1 bump (#341), and a large consolidation/cleanup batch — `admin.py` split into a domain-scoped package (#345), bank-review views and year-filter/overdue-days logic unified (#346, #354), CSS token unification for tag/status-badge palettes (#352, #353), remaining i18n blind spots and a missing `ClientInquiry.status` index closed (#356, #359, #360), test coverage added for two management commands and 18 admin classes (#357). Full list: [docs/CHANGELOG.md](docs/CHANGELOG.md).
- **2026-08-18 — Third review pass (i18n/tests/forms/schema cleanup)**: five follow-up PRs from a third codebase audit (test suite quality, i18n blind spots, forms consistency, migration integrity). i18n: wrapped the 8 stage-email-template labels in `inquiry_views.py` and an unwrapped f-string in `bank_import.py` that the `test_i18n_coverage.py` guardrail can't catch (it only scans template source, not Python strings fed into an already-wrapped template); filled in a missing German plural translation; closed `verbose_name`/`Meta.verbose_name` i18n gaps on `Practice` (10 address/contact/banking fields), `CompanyWithdrawal`/`CompanyExpense` (raw bilingual-slash strings), and `PendingCalendarEvent.google_event_id`. Tests: added coverage for the previously-untested `remove_financial_duplicates`/`compress_media` management commands and 18 of 25 admin classes. Forms: `save_tax_year_note`'s hand-rolled Decimal/date parsing replaced with a proper `TaxYearNoteForm`, preserving the endpoint's partial-update contract shared by two independent widgets. Schema: added a missing index on `ClientInquiry.status`; deliberately did **not** add the originally-planned "one draft Invoice per client" / "one active GoogleCalendarToken per practice" DB constraints after finding the first premise didn't hold against the actual (unguarded) invoice-creation code paths and the second carried real migration-apply risk that couldn't be verified in a sandboxed environment. `docs/architecture/PERFORMANCE.md`'s migration-number references corrected (0013/0014 → squashed into `0001_initial.py`; the `InvoiceItem.session_date` index no longer exists at all — the field was replaced by a `session` FK).
