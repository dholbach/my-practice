...
- ✅ Shut-D (Shutdown Dissociation Scale) shipped in-repo as a second public instrument (Schalinski et al. 2016, CC BY-SA 4.0); added section-level `note` field to the questionnaire schema for interviewer-only guidance text, distinct from the public `intro` paragraph
