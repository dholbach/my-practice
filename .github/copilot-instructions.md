...
./dev.py test payments_app.tests.test_calculations  # Specific test
...
### Modular Structure
- **Models** (`app/payments_app/models/`): Domain-focused modules (client, invoice, practice, financial, etc.)
- **Views** (`app/payments_app/views/`): Feature-specific view modules + CRUD mixins
- **Utils** (`app/payments_app/utils/`): Reusable business logic - **USE THESE FIRST!**
...
from payments_app.utils import FinancialListContextBuilder
...
# Analytics dashboard
from payments_app.utils import AnalyticsDashboardBuilder
...
```python
from payments_app.utils import InvoiceFilterHelper
...
```python
from payments_app.utils import count_sessions
...
class ExpenseCreateView(FormCreateViewMixin):
    ...
    template_name = "payments_app/expense_form.html"
...
from payments_app.utils import RevenueCalculator
...
# Date ranges
from payments_app.utils import DateRangeHelper
helper = DateRangeHelper(start_date, end_date)
months = helper.get_month_list()

# Client helpers
from payments_app.utils import build_client_map
...
# Avoid
from payments_app.models import Client
```
...
**Rule: No inline `<style>` blocks in templates — all CSS belongs in static files.**

### Structure
- `common.css` — global utilities, form classes, badges, tags, layout helpers (loaded globally via `base.html`)
- `widgets.css` — dashboard widget chrome and agenda styles (loaded globally via `base.html`)
- Per-page CSS — named after the template: `dashboard.css`, `todo_list.css`, `client_list.css`, etc.
- PDF templates (`invoice_pdf_*.html`) are exempt — they require inline styles for PDF rendering

### Adding page-specific styles
Always use `{% block extra_css %}` with a `<link>` tag:
```html
{% block extra_css %}
...
```python
from payments_app.utils.date_helpers import DateRangeHelper
from payments_app.utils.practice_days import berlin_public_holidays
